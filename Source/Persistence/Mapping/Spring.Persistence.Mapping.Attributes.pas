...
begin
  Result := fMember.HasCustomAttribute<AutoGenerated>;
end;
...
begin
  ...
  if fMember is TRttiField then
    Result := TRttiField(fMember).FieldType.Handle
  else if fMember is TRttiProperty then
    Result := TRttiProperty(fMember).PropertyType.Handle;
end;
...
procedure ManyValuedAssociation.SetEntityClass(const Value: TClass);
var
  rttiType: TRttiType;
begin
  inherited;
