...
///	<summary>Introduces cloning into Spring.</summary>
unit Spring.Cloning;
interface
uses TypInfo,
     Rtti,
...
type
  ...
  TReplicator<T> = class sealed(TInterfacedObject)
  private type
  ...
  private
    ...
    FObjectMap, FRecordMap,
      FDynArrayMap: IDictionary<Pointer, Pointer>;
...
{$HINTS ON}
  ...
  TCloneableObject = class(TInterfacedObject, ICloneable)
  private
    [CloneKind(ckSkip)]
...
begin
  ...
  if FObjectMap = nil then
    FObjectMap := TCollections.CreateDictionary<Pointer, Pointer>();
...
begin
  { Create the dictionary if it's not created yet. }
  if FRecordMap = nil then
    FRecordMap := TCollections.CreateDictionary<Pointer, Pointer>();
