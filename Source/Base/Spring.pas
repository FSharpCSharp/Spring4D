...
{$IFDEF DELPHIXE7_UP}
  ...
  protected
    ...
    class procedure DownHeap_Swap_Ref(values: PByte; hi: Integer; {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; i: Integer; size: Integer); static;
    class procedure HeapSort_Swap_Ref(values: PByte; hi: Integer; {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer); static;
    class procedure InsertionSort_Swap_Ref(values: PByte; hi: Integer; {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer); static;
    class function QuickSortPartition_Swap_Ref(values: PByte; hi: Integer; {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer): Integer; static;
    class procedure IntroSort_Swap_Ref(values: PByte; hi: Integer; {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer; depthLimit: Integer = -1); static;
    ...
    class procedure IntroSort_Pointer(const values: Pointer; hi: Integer; const compare: PMethod); static;
  public
    /// <summary>
...
class procedure TArray.DownHeap_Swap_Ref(values: PByte; hi: Integer;
...
class procedure TArray.HeapSort_Swap_Ref(values: PByte; hi: Integer;
  {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer);
var
  i: Integer;
begin
  for i := (hi+1) shr 1 - 1 downto 0 do
    DownHeap_Swap_Ref(values, hi, compare, i, size);
  for i := hi downto 1 do
  begin
    BinarySwap(@values[i*size], @values[0], size);
    DownHeap_Swap_Ref(values, i-1, compare, 0, size);
  end;
end;

class procedure TArray.InsertionSort_Swap_Ref(values: PByte; hi: Integer;
  {$IFDEF SUPPORTS_CONSTREF}[ref]{$ENDIF}const compare: TCompareMethod; size: Integer);
var
  i, j: Integer;
begin
...
class function TArray.QuickSortPartition_Swap_Ref(values: PByte; hi: Integer;
...
class procedure TArray.IntroSort_Swap_Ref(values: PByte; hi: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      InsertionSort_Swap_Ref(values, partitionSize - 1, compare, size);
      Exit;
    end
    else
    begin
      if depthLimit = 0 then
      begin
        HeapSort_Swap_Ref(values, partitionSize - 1, compare, size);
        Exit;
      end;
      ...
      Dec(depthLimit);
      pivot := QuickSortPartition_Swap_Ref(values, partitionSize - 1, compare, size);
      {$R-}
      IntroSort_Swap_Ref(@values[(pivot + 1) * size], partitionSize - (pivot + 1) - 1, compare, size, depthLimit);
      {$IFDEF RANGECHECKS_ON}{$R+}{$ENDIF}
      partitionSize := pivot;
    end;
  end;
...
class procedure TArray.IntroSort_Pointer(const values: Pointer; hi: Integer; const compare: PMethod);
begin
  {$Q-}{$R-}
  {$IF SizeOf(Pointer) = 4}
  IntroSort<Int32>(Slice(TSlice<Int32>(values^), hi + 1), TCompareMethod<Int32>(compare^));
    ...
    case GetTypeKind(T) of
      ...
      tkFloat:
        case SizeOf(T) of
          ...
          10: IntroSort_Extended(@values[0], High(values), @compare);
        else
      ...
      tkString:
        IntroSort_Swap_Ref(@values[0], High(values), TCompareMethod(compare), SizeOf(T));
      tkSet:
        case SizeOf(T) of
          1: IntroSort_Int8(@values[0], High(values), @compare);
          2: IntroSort_Int16(@values[0], High(values), @compare);
    ...
    case GetTypeKind(T) of
      ...
      tkFloat:
        case SizeOf(T) of
          ...
          10: IntroSort_Extended(@values[index], index + count - 1, @compare);
        else
          if GetTypeData(TypeInfo(T)).FloatType = ftDouble then
      ...
      tkString:
        IntroSort_Swap_Ref(@values[index], index + count - 1, TCompareMethod(compare), SizeOf(T));
      tkSet:
        case SizeOf(T) of
          1: IntroSort_Int8(@values[index], index + count - 1, @compare);
          2: IntroSort_Int16(@values[index], index + count - 1, @compare);
...
begin
  ...
  comparer := _LookupVtableInfo(giComparer, TypeInfo(TObject), SizeOf(TObject));
  TArray.Compare_Object.Data := comparer;
  TArray.Compare_Object.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(TMethodPointer), SizeOf(TMethodPointer));
  TArray.Compare_Method.Data := comparer;
  TArray.Compare_Method.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(IInterface), SizeOf(IInterface));
  TArray.Compare_Interface.Data := comparer;
  TArray.Compare_Interface.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(string), SizeOf(string));
  TArray.Compare_String.Data := comparer;
  TArray.Compare_String.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(Int8), SizeOf(Int8));
  TArray.Compare_Int8.Data := comparer;
  TArray.Compare_Int8.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(Int16), SizeOf(Int16));
  TArray.Compare_Int16.Data := comparer;
  TArray.Compare_Int16.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(Int32), SizeOf(Int32));
  TArray.Compare_Int32.Data := comparer;
  TArray.Compare_Int32.Code := PPVTable(comparer)^[3];
  comparer := _LookupVtableInfo(giComparer, TypeInfo(Int64), SizeOf(Int64));
  TArray.Compare_Int64.Data := comparer;
  TArray.Compare_Int64.Code := PPVTable(comparer)^[3];
end;
