...
begin
  ...
  begin
    if (dependency.DependencyType = dtParameter) or 
      not fRegistry.HasServiceType(dependency.TargetTypeInfo) then
    begin
      Result := False;
      Break;
    end;
...
begin
  ...
  for dependency in member.GetDependencies do
  begin
    if fDependencies.Contains(dependency.TargetTypeInfo) then
    begin
      raise ECircularDependencyException.CreateResFmt(
        @SCircularDependencyDetected, 
        [dependency.TargetType.Name]
      );
    end;
    model := fRegistry.FindComponentModel(dependency.TargetTypeInfo);
    if model = nil then
    begin
      raise EResolveException.CreateResFmt(@SCannotResolveDependency, [dependency.TargetType.Name]);
    end;
    fDependencies.Add(dependency.TargetTypeInfo);
    try
      instance := model.LifetimeManager.GetInstance;
    finally
      fDependencies.Remove(dependency.TargetTypeInfo);
    end;
    if dependency.TargetType.IsClass then
    begin
      Result[index] := instance;
    end
    else if dependency.TargetType.IsInterface then
    begin
      instance.GetInterface(GetTypeData(dependency.TargetTypeInfo).Guid, localInterface);
      TValue.MakeWithoutCopy(@localInterface, dependency.TargetTypeInfo, Result[index]);
    end
    else
    begin
...
begin
  ...
  Result := nil;
  model := fRegistry.FindComponentModel(typeInfo);
  if model = nil then
  begin
