...
type
  ...
  /// </summary>
  IValidator = interface
    ['{AFA2E652-84C9-48E0-98C5-2C0E3572C7C7}']
  ...
  IValueValidator = interface(IValidator)
    procedure Initialize(const provider: IValueProvider);
  end;
...
//  end;
  ...
  TValidatorBase = class(TInterfacedObject, IValidator, IInterface)
  private
    fIsValid: Boolean;
  ...
  TMinValueValidator = class(TValidatorBase)
  private
    fMinValue: Variant;
  protected
    ...
    function DoValidate(const value: Variant; out errorMessage: string): Boolean; override;
  public
    constructor Create(const provider: IValueProvider; const minValue: Variant);
    property MinValue: Variant read fMinValue;
  end;

  TMaxValueValidator = class(TValidatorBase)
  private
    fMaxValue: Variant;
  protected
    function DoAccept(const value: Variant): Boolean; override;
    function DoValidate(const value: Variant; out errorMessage: string): Boolean; override;
  public
    constructor Create(const provider: IValueProvider; const maxValue: Variant);
  ...
  TValidationAttributeBase = class abstract(TCustomAttribute)
  public
    function CreateValidator(const provider: IValueProvider): IValidator; virtual; abstract;
  end;
...
{$REGION 'TMinValueValidator'}

constructor TMinValueValidator.Create(const provider: IValueProvider;
  const minValue: Variant);
begin
  inherited Create(provider);
  fMaxValue := maxValue;
end;

function TMinValueValidator.DoAccept(const value: Variant): Boolean;
begin
  Result := not VarIsNullOrEmpty(value) and not VarIsNullOrEmpty(Self.MinValue);
end;

function TMinValueValidator.DoValidate(const value: Variant;
  out errorMessage: string): Boolean;
begin
...
{$REGION 'TMaxValueValidator'}

constructor TMaxValueValidator.Create(const provider: IValueProvider;
  const maxValue: Variant);
begin
  inherited Create(provider);
  fMaxValue := maxValue;
end;

function TMaxValueValidator.DoAccept(const value: Variant): Boolean;
begin
  Result := not VarIsNullOrEmpty(value) and not VarIsNullOrEmpty(Self.MaxValue);
end;

function TMaxValueValidator.DoValidate(const value: Variant;
  out errorMessage: string): Boolean;
begin
